...
export class GlowfinAudio {
  ...
  private createAmbientGraph(context: AudioContext): void {
    ...
    waterFilter.frequency.value = 840;
    ...
    this.ambientSources.push(bed, current, shimmer, water);
  ...
  private playCue(cue: GameplayAudioCue): void {
    ...
    switch (cue.type) {
      ...
      case "collision":
        if (!this.hasCueCapacity(2)) return;
        this.tone(138.59, 55, 0.48, 0.26, "triangle", pan);
        this.noiseBurst(0.38, 0.2, 190, -pan);
        return;
      case "recovery":
        if (!this.hasCueCapacity(2)) return;
      ...
      case "run-end":
        if (!this.hasCueCapacity(2)) return;
        this.tone(220, 110, 0.72, 0.16, "sine", -0.12);
        this.tone(164.81, 82.41, 0.82, 0.12, "triangle", 0.12, 0.08);
        return;
    }
  }
  ...
  private renderUiState(): void {
    ...
    const enabled = !this.muted && this.uiState !== "unavailable";
    this.button.setAttribute("aria-pressed", String(enabled));

    const messages: Record<AudioUiState, string> = {
      locked: "Sound on — starts with your first touch",
      active: "Sound on",
      muted: "Sound off",
