...
state. The 64-second Moon-Current score is the musical foreground; quiet
beat-pulsed current and harmonic-shimmer support enters as momentum builds.

| Key | Meaning |
|---|---|
| `masterGain` | Final output ceiling for the complete Web Audio graph. |
| `ambientGain` | Shared budget for the fixed Web Audio support layers behind the native score. |
...
One native media stream carries Moon-Current through the physical phone media
pipeline. The Web Audio graph is not created until a real pointer gesture,
satisfying iOS Safari's autoplay policy. Muting suspends both paths and the
choice is stored only on the device. Audio failure must leave the game fully
playable and must never trigger the startup-error surface.
