...
|---|---|---|
| `local` | developer iteration | `V36 · LOCAL · local` |
| `staging` | owner-only certification checkpoint | exact Git commit SHA and artifact digest |
| `production` | promoted public release | exact `glowfin-v36` tagged commit and unchanged artifact digest |
...
1. Merge only after the PR's Core CI, structural, phone-render, touch-audio and
   lifecycle gates are green, plus the Phase 5B fault/privacy/rollback gate.
...
5. Confirm the rollback manifest is the saved Version 35 artifact, not a rebuild.
